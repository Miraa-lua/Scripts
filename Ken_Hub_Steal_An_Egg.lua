if not game then
    error("Ken Hub: Run this script in a Roblox executor, not a Lua editor.")
end

if type(writefile) == "function" then
    pcall(function()
        writefile("LSS_RESUME.lua", [[
local path = "LSSmainfix_autotreadmill_CLEAN.lua"
...
local function LSS_BOOT_PARENT()
    local parent = nil
    pcall(function()
...
local function LSS_BOOT_CLEAR()
    pcall(function()
        local parent = LSS_BOOT_PARENT()
        local old = parent and parent:FindFirstChild("LSS_BOOT_PANEL")
        if old then old:Destroy() end
    end)
end

local function LSS_BOOT_PANEL(title, body)
    local parent = LSS_BOOT_PARENT()
    if not parent then return end
    local panel = parent:FindFirstChild("LSS_BOOT_PANEL")
    if not panel then
        panel = Instance.new("ScreenGui")
        panel.Name = "LSS_BOOT_PANEL"
    ...
    if card then
        ...
        if titleLabel then titleLabel.Text = tostring(title or "Ken Hub") end
        if bodyLabel then bodyLabel.Text = tostring(body or "Loading...") end
    end
end

LSS_BOOT_PANEL("Ken Hub", "Loading script...")

local __LSS_BOOT_OK, __LSS_BOOT_ERR = xpcall(function()

do
    local prev = _G.LSSStealAnEgg
...
pcall(function()
    ...
    for _, parent in ipairs(parentList) do
        if parent then
            local previous = parent:FindFirstChild("LSS_StealMini_V45")
...
if not okWindUI or not WindUI then
    error("Ken Hub: Failed to download WindUI " .. WindUI_VERSION)
end
pcall(function() WindUI:SetNotificationLower(true) end)
LSS_BOOT_PANEL("Ken Hub", "WindUI loaded, building UI...")
local LSS_TEXT = {
    ["LSS"] = "Ken Hub",
    ["LSS | Steal An Egg"] = "Ken Hub | Steal An Egg",
    ["Eggs"] = "Eggs",
...
local function LSS_WRAP_TAB(rawTab)
    ...
    function tab:AddParagraph(opts)
        opts = opts or {}
        pcall(function() rawTab:Paragraph({Title = VI(opts.Title or "Ken Hub"), Desc = VI(opts.Content or "")}) end)
    end
    return tab
end
...
if not okWindUIWindow or not WindUIWindow then
    error("Ken Hub: WindUI CreateWindow error: " .. tostring(WindUIWindow))
end
...
LSS_BOOT_PANEL("Ken Hub", "WindUI built, loading tabs...")
local Library = {}
...
function Library:CreateWindow()
    ...
    function win:Notify(opts)
        opts = opts or {}
        pcall(function() WindUI:Notify({Title = opts.Title or "Ken Hub", Content = opts.Content or "", Duration = opts.Duration or 3}) end)
    end
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then
            pcall(Notify, "Ken Hub", "Error: " .. tostring(err), "Error", 4)
        end
    end
end
...
function api:Run(record,method,speed)
    ...
    assert(record and record.Uid and record.BoundsCFrame,"LSS selected egg is missing Uid/BoundsCFrame")
    transferTarget={
        Uid=record.Uid,
...
local function StealSpecificEggRobust(targetItem)
    if HUB.dead then return false,"LSS is unloaded" end
    if HUB.paused then return false,"LSS is paused" end
...
local function StealBestEggOnce()
    ...
    if #eggs==0 then return false,"No eggs match the LSS filter" end
...
function HUB.GuardRide.GetAnimator(model)
    ...
    if not host then
        ...
        host.Name = "LSS_RideAnimationController"
        host.Parent = model
    end
...
function HUB.GuardRide.SetupAnimations(source, clone)
    ...
    HUB.GuardRide.moveTrack = loadCandidate(bestMove, "LSS_GuardRun_PRELOADED", Enum.AnimationPriority.Movement)
    HUB.GuardRide.idleTrack = loadCandidate(bestIdle, "LSS_GuardIdle_PRELOADED", Enum.AnimationPriority.Idle)
    ...
    if sourceAnimator then
        HUB.GuardRide.sourceAnimConn = sourceAnimator.AnimationPlayed:Connect(function(track)
            ...
            if looksMove(context) or priority == Enum.AnimationPriority.Movement then
                local replacement = loadCandidate({id = anim.AnimationId}, "LSS_GuardRun_CAPTURED", Enum.AnimationPriority.Movement)
                if replacement then
            ...
            elseif (looksIdle(context) or priority == Enum.AnimationPriority.Idle) and not HUB.GuardRide.idleTrack then
                HUB.GuardRide.idleTrack = loadCandidate({id = anim.AnimationId}, "LSS_GuardIdle_CAPTURED", Enum.AnimationPriority.Idle)
...
function HUB.GuardRide.Start()
    ...
    clone.Name = "LSS_LightDark_GuardRide_REALMOVE"
    ...
    avatar.Name = "LSS_GuardRide_RigidAvatar"
...
function HUB.SetEnabled(value)
    ...
    if shouldPause then
        ...
        Notify("LSS", "Paused; UI remains open to re-enable.", "Info", 3)
    else
...
HUB.UI.SettingsTab:AddToggle({
    Name = "Auto re-run on server hop", Flag = "settings_resume", Default = LSS_CFG.autoResume,
    Callback = function(on)
        LSS_CFG.autoResume = on == true
        LSS_CFG.Save(true)
        if LSS_CFG.autoResume then LSS_CFG.Queue() end
    end
})
HUB.UI.SettingsTab:AddToggle({
    Name = "Auto save settings", Flag = "settings_autosave", Default = LSS_CFG.autoSave,
    Callback = function(on)
        LSS_CFG.autoSave = on == true
        LSS_CFG.Save(true)
    end
})
HUB.UI.SettingsTab:AddButton({
    ...
    Callback = safeCallback(function()
        local ok, err = LSS_CFG.Save(true)
...
HUB.UI.SettingsTab:AddButton({
    Name = "Load config",
    Callback = safeCallback(function()
        local ok, err = LSS_CFG.LoadAndApply()
        Notify("Settings", ok and "Loaded" or ("Load failed: " .. tostring(err)), ok and "Success" or "Error")
    end)
})
HUB.UI.SettingsTab:AddToggle({
    Name = "Enable LSS activity", Default = true, Flag = "phucmax_enabled",
...
pcall(function()
    track(LP.OnTeleport:Connect(function()
        if LSS_CFG.autoSave then LSS_CFG.Save(true) end
    end))
end)
...
Notify("LSS", "Script loaded successfully!", "Success", 3.5)
if LSS_CREATED_TABS <= 0 then
    LSS_BOOT_PANEL("LSS UI WARNING", "Script finished but no UI tab was created.")
else
    LSS_BOOT_CLEAR()
end
...
if not __LSS_BOOT_OK then
    LSS_BOOT_PANEL("LSS UI ERROR", tostring(__LSS_BOOT_ERR))
end